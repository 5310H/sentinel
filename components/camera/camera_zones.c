 ...
 * Cameras with motion detection enabled become virtual zones that can trigger alarms.
 *
...
#define ESP_LOGW(tag, ...) printf("[%s] WARNING: ", tag); printf(__VA_ARGS__); printf("\n")
#define ESP_LOGE(tag, ...) printf("[%s] ERROR: ", tag); printf(__VA_ARGS__); printf("\n")
#define ESP_LOGD(tag, ...) printf("[%s] DEBUG: ", tag); printf(__VA_ARGS__); printf("\n")
#endif
...
esp_err_t camera_zones_register_camera(int camera_id) {
    ...
    for (int i = 0; i < MAX_CAMERA_ZONES; i++) {
        if (!camera_zones[i].active) {
            ...
            ESP_LOGI(TAG, "Registered camera %d as zone %d", camera_id, camera_zones[i].zone_index);
            return ESP_OK;
        }
    }
...
esp_err_t camera_zones_unregister_camera(int camera_id) {
    for (int i = 0; i < MAX_CAMERA_ZONES; i++) {
        if (camera_zones[i].active && camera_zones[i].camera_id == camera_id) {
            ...
            ESP_LOGI(TAG, "Unregistered camera %d from zone %d", camera_id, camera_zones[i].zone_index);
            return ESP_OK;
        }
    }
...
void camera_zones_poll_all(void) {
    ...
    for (int i = 0; i < MAX_CAMERA_ZONES; i++) {
        ...
        if (motion_detected) {
            ...
            if (engine_get_arm_state_safe() == 2 || engine_get_arm_state_safe() == 3) { // Armed away or stay
                ESP_LOGI(TAG, "Camera zone %d triggered by camera %d", zone_index, camera_id);
