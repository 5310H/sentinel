...
esp_err_t camera_mgr_init(void) {
  ...
  initialized = true;
  ESP_LOGI(TAG, "Camera manager initialized (max %d devices)",
           CAMERA_MAX_DEVICES);
...
esp_err_t camera_mgr_load_config(void) {
...
#ifdef ESP_PLATFORM
  char *buffer =
      heap_caps_malloc(fsize + 1, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#else
  ...
  cJSON_ArrayForEach(item, root) {
    ...
    strncpy(cam->friendly_name, name->valuestring,
            sizeof(cam->friendly_name) - 1);
    if (esphome_id && cJSON_IsString(esphome_id)) {
      strncpy(cam->esphome_id, esphome_id->valuestring,
              sizeof(cam->esphome_id) - 1);
    } else {
      strcpy(cam->esphome_id, "camera");
    }
    cam->enabled = enabled ? cJSON_IsTrue(enabled) : true;
    cam->sd_recording_enabled =
        sd_recording ? cJSON_IsTrue(sd_recording) : sd_card_available;

    // Load custom snapshot command if provided
    if (snapshot_cmd && cJSON_IsString(snapshot_cmd)) {
      strncpy(cam->snapshot_cmd, snapshot_cmd->valuestring,
              sizeof(cam->snapshot_cmd) - 1);
    ...
    cJSON *motion_enabled =
        cJSON_GetObjectItem(item, "motion_detection_enabled");
    if (motion_cmd_json && cJSON_IsString(motion_cmd_json)) {
      strncpy(cam->motion_cmd, motion_cmd_json->valuestring,
              sizeof(cam->motion_cmd) - 1);
      cam->motion_detection_enabled =
          motion_enabled ? cJSON_IsTrue(motion_enabled) : true;
      ESP_LOGI(TAG, "Motion detection enabled for %s: %s", cam->friendly_name,
               cam->motion_cmd);
    } else {
      cam->motion_cmd[0] = '\0';
...
esp_err_t camera_mgr_save_config(void) {
  ...
  for (int i = 0; i < camera_count; i++) {
    ...
    cJSON_AddBoolToObject(item, "sd_recording_enabled",
                          cameras[i].sd_recording_enabled);
    ...
    if (cameras[i].motion_detection_enabled &&
        strlen(cameras[i].motion_cmd) > 0) {
      cJSON_AddStringToObject(item, "motion_cmd", cameras[i].motion_cmd);
      cJSON_AddBoolToObject(item, "motion_detection_enabled",
                            cameras[i].motion_detection_enabled);
    }
...
int camera_mgr_add_device(const char *hostname, int port,
                          const char *friendly_name, const char *esphome_id,
                          const char *username, const char *password,
                          const char *snapshot_cmd, const char *motion_cmd,
                          bool enabled) {
  if (!initialized)
    return -1;
  if (camera_count >= CAMERA_MAX_DEVICES) {
  ...
  strncpy(cam->esphome_id, esphome_id ? esphome_id : "camera",
          sizeof(cam->esphome_id) - 1);
  strncpy(cam->username, username ? username : "", sizeof(cam->username) - 1);
  strncpy(cam->password, password ? password : "", sizeof(cam->password) - 1);
  strncpy(cam->snapshot_cmd, snapshot_cmd ? snapshot_cmd : "",
          sizeof(cam->snapshot_cmd) - 1);
  strncpy(cam->motion_cmd, motion_cmd ? motion_cmd : "",
          sizeof(cam->motion_cmd) - 1);
  cam->enabled = enabled;
  cam->sd_recording_enabled =
      sd_card_available; // Enable by default if SD available
  cam->motion_detection_enabled =
      (motion_cmd && strlen(motion_cmd) > 0); // Enable if motion_cmd provided
  cam->snapshot_buffer = NULL;
  cam->snapshot_size = 0;
  cam->last_snapshot_time = 0;
  cam->motion_detected = false;
  ...
  ESP_LOGI(TAG, "Added camera %d: %s (%s:%d)", camera_id, friendly_name,
           hostname, cam->port);
 ...
 */
int camera_mgr_get_count(void) { return camera_count; }
...
bool camera_mgr_poll_motion(int camera_id) {
    ...
    if (status_code == 200) {
      ...
      if (content_length > 0 && content_length < sizeof(response_buffer)) {
        ...
        if (read_len > 0) {
          ...
          // - HTTP status based (some cameras return 200 for motion, 204 for no
          // motion)
          ...
          if (root) {
            ...
            if (motion) {
              ...
              if (motion) {
                motion_detected =
                    cJSON_IsTrue(motion) ||
                    (cJSON_IsNumber(motion) && motion->valueint != 0);
              }
            }
            cJSON_Delete(root);
          } else {
          ...
          ESP_LOGD(TAG, "Motion poll response: %s -> %s", response_buffer,
                   motion_detected ? "MOTION" : "no motion");
        }
      }

      cam->consecutive_failures = 0; // Reset failure count on success
    } else {
      ESP_LOGW(TAG, "Motion poll failed for camera %d: HTTP %d", camera_id,
               status_code);
      cam->consecutive_failures++;
    }
  } else {
    ESP_LOGW(TAG, "Motion poll network error for camera %d: %s", camera_id,
             esp_err_to_name(err));
    cam->consecutive_failures++;
  }
  ...
  if (motion_detected) {
    ...
    ESP_LOGI(TAG, "Motion detected by camera %d (%s)", camera_id,
             cam->friendly_name);
  }
  ...
  if (now - last_motion[camera_id] > 30 + (rand() % 90)) {
    ...
    ESP_LOGI(TAG, "[MOCK] Motion detected by camera %d (%s)", camera_id,
             cam->friendly_name);
    return true;
  } else {
    cam->motion_detected = false;
...
esp_err_t camera_mgr_set_motion_detection(int camera_id, bool enabled) {
  ...
  ESP_LOGI(TAG, "Camera %d motion detection %s", camera_id,
 ...
 */
esp_err_t camera_mgr_fetch_snapshot(int camera_id, uint8_t **out_buffer,
                                    size_t *out_size) {
  if (camera_id < 0 || camera_id >= camera_count) {
    return ESP_ERR_INVALID_ARG;
  }
  ...
  if (strlen(cam->snapshot_cmd) > 0) {
    ...
    if (err == ESP_OK) {
      ...
      if (status_code == 200) {
        int content_length = esp_http_client_get_content_length(client);
        if (content_length > 0 &&
            content_length <= CAMERA_SNAPSHOT_BUFFER_SIZE) {
          // Allocate buffer
#ifdef ESP_PLATFORM
          uint8_t *new_buffer =
              heap_caps_realloc(cam->snapshot_buffer, content_length,
                                MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#else
          uint8_t *new_buffer = realloc(cam->snapshot_buffer, content_length);
#endif
          if (!new_buffer) {
            ESP_LOGE(TAG, "Failed to allocate %d bytes for snapshot",
                     content_length);
            esp_http_client_cleanup(client);
            return ESP_ERR_NO_MEM;
          }
        ...
        while (total_read < content_length) {
          int read_len = esp_http_client_read(
              client, (char *)cam->snapshot_buffer + total_read,
...
for (int pattern_idx = 0; pattern_idx < 4; pattern_idx++) {
  ...
  if (pattern_idx == 0) {
    ...
    snprintf(url, sizeof(url), url_patterns[pattern_idx], cam->hostname,
             cam->port, cam->esphome_id);
  } else {
  ...
  if (err != ESP_OK) {
    ESP_LOGW(TAG, "HTTP request failed for pattern %d: %s", pattern_idx + 1,
             esp_err_to_name(err));
    esp_http_client_cleanup(client);
    continue;
  }
  ...
  int status_code = esp_http_client_get_status_code(client);
  if (status_code != 200) {
    ESP_LOGW(TAG, "HTTP error %d from pattern %d", status_code,
             pattern_idx + 1);
    esp_http_client_cleanup(client);
    continue;
  }
  ...
  while (total_read < content_length) {
    int read_len =
        esp_http_client_read(client, (char *)cam->snapshot_buffer + total_read,
                             content_length - total_read);
    if (read_len <= 0)
      break;
    total_read += read_len;
 ...
 * Get cached snapshot
 */
esp_err_t camera_mgr_get_cached_snapshot(int camera_id, uint8_t **out_buffer,
                                         size_t *out_size) {
  if (camera_id < 0 || camera_id >= camera_count) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t camera_mgr_save_snapshot_to_sd(int camera_id, int zone_id,
  ...
  snprintf(filename, sizeof(filename),
           ...
           ALARM_SNAPSHOT_DIR, timeinfo.tm_year + 1900, timeinfo.tm_mon + 1,
  ...
  if (written != size) {
    ESP_LOGE(TAG, "Failed to write complete snapshot: %d/%d bytes", written,
             size);
    return ESP_FAIL;
  }
 ...
 * Check if SD card is available
 */
bool camera_mgr_is_sd_available(void) { return sd_card_available; }
...
esp_err_t camera_mgr_set_sd_recording(int camera_id, bool enabled) {
  ...
  ESP_LOGI(TAG, "Camera %d SD recording %s", camera_id,
           enabled ? "enabled" : "disabled");
