...
// Global instances defined in .h
config_t config;
relay_t relays[MAX_RELAYS];
user_t users[MAX_USERS];
zone_t zones[MAX_ZONES];
esphome_device_t esphome_devices[32];
network_t net_cfg;

int z_count = 0, r_count = 0, u_count = 0, esphome_count = 0;

// --- INTERNAL HELPERS ---

static void get_full_path(char *dest, const char *filename) {
    const char *offset = (strncmp(filename, "data/", 5) == 0) ? filename + 5 : filename;
...
static int safe_get_int(cJSON *obj, int fallback) {
    if (!obj || obj->type != cJSON_Number) return fallback;
    return obj->valueint;
}

static uint32_t safe_get_uint32(cJSON *obj, uint32_t fallback) {
    if (!obj || obj->type != cJSON_Number) return fallback;
    if (obj->valuedouble > UINT32_MAX) return UINT32_MAX;
...
zone_type_t zone_type_from_string(const char *type_str) {
    if (type_str == NULL) return ZONE_TYPE_UNKNOWN;
    
    if (strcmp(type_str, "door") == 0) return ZONE_TYPE_DOOR;
    if (strcmp(type_str, "window") == 0) return ZONE_TYPE_WINDOW;
    if (strcmp(type_str, "motion") == 0) return ZONE_TYPE_MOTION;
    if (strcmp(type_str, "camera") == 0) return ZONE_TYPE_CAMERA;
    if (strcmp(type_str, "glass_break") == 0) return ZONE_TYPE_GLASS_BREAK;
    if (strcmp(type_str, "fire") == 0) return ZONE_TYPE_FIRE;
    if (strcmp(type_str, "smoke") == 0) return ZONE_TYPE_SMOKE;
    if (strcmp(type_str, "panic") == 0) return ZONE_TYPE_PANIC;
    if (strcmp(type_str, "medical") == 0) return ZONE_TYPE_MEDICAL;
    if (strcmp(type_str, "temp") == 0) return ZONE_TYPE_TEMP;
    if (strcmp(type_str, "water") == 0) return ZONE_TYPE_WATER;
...
const char* zone_type_to_string(zone_type_t type) {
    switch (type) {
        ...
        case ZONE_TYPE_WINDOW: return "window";
        case ZONE_TYPE_MOTION: return "motion";
        case ZONE_TYPE_CAMERA: return "camera";
        case ZONE_TYPE_GLASS_BREAK: return "glass_break";
        case ZONE_TYPE_FIRE: return "fire";
        case ZONE_TYPE_SMOKE: return "smoke";
        case ZONE_TYPE_PANIC: return "panic";
        case ZONE_TYPE_MEDICAL: return "medical";
        case ZONE_TYPE_TEMP: return "temp";
        case ZONE_TYPE_WATER: return "water";
        default: return "unknown";
    }
}

zone_call_t zone_call_from_string(const char *type_str) {
    if (type_str == NULL) return ZONE_CALL_UNKNOWN;   
    if (strcmp(type_str, "fire") == 0) return ZONE_CALL_FIRE;
    if (strcmp(type_str, "police") == 0) return ZONE_CALL_POLICE;
    if (strcmp(type_str, "medical") == 0) return ZONE_CALL_MEDICAL;
    if (strcmp(type_str, "other") == 0) return ZONE_CALL_OTHER;
    return ZONE_CALL_UNKNOWN;
}

const char* zone_call_to_string(zone_call_t type) {
    switch (type) {
        case ZONE_CALL_FIRE: return "fire";
        case ZONE_CALL_POLICE: return "police";
        case ZONE_CALL_MEDICAL: return "medical";
        case ZONE_CALL_OTHER: return "other";
        default: return "unknown";
    }
}

relay_type_t relay_type_from_string(const char *type_str) {
    if (type_str == NULL) return RELAY_TYPE_UNKNOWN;
    
    if (strcmp(type_str, "alarm") == 0) return RELAY_TYPE_ALARM;
    if (strcmp(type_str, "siren") == 0) return RELAY_TYPE_SIREN;
    if (strcmp(type_str, "strobe") == 0) return RELAY_TYPE_STROBE;
    if (strcmp(type_str, "chime") == 0) return RELAY_TYPE_CHIME;
    if (strcmp(type_str, "bell") == 0) return RELAY_TYPE_BELL;
    if (strcmp(type_str, "light") == 0) return RELAY_TYPE_LIGHT;
    if (strcmp(type_str, "door_lock") == 0) return RELAY_TYPE_DOOR_LOCK;
    if (strcmp(type_str, "gate") == 0) return RELAY_TYPE_GATE;
...
const char* relay_type_to_string(relay_type_t type) {
    switch (type) {
        case RELAY_TYPE_ALARM: return "alarm";
        case RELAY_TYPE_SIREN: return "siren";
        case RELAY_TYPE_STROBE: return "strobe";
        case RELAY_TYPE_CHIME: return "chime";
        case RELAY_TYPE_BELL: return "bell";
        case RELAY_TYPE_LIGHT: return "light";
        case RELAY_TYPE_DOOR_LOCK: return "door_lock";
        case RELAY_TYPE_GATE: return "gate";
        default: return "unknown";
    }
}
...
esp_err_t init_spiffs(void) {
    ...
    esp_vfs_spiffs_conf_t conf = {
        .base_path = "/spiffs",
        .partition_label = "storage", // Matches your partitions.csv
        .max_files = 5,
    ...
    if (ret != ESP_OK) {
        if (ret == ESP_FAIL) {
            ...
            ESP_LOGE("STORAGE", "Failed to initialize SPIFFS (%s)", esp_err_to_name(ret));
        }
        return ret;
    }
    ...
    if (ret != ESP_OK) {
        ESP_LOGE("STORAGE", "Failed to get partition info (%s)", esp_err_to_name(ret));
    } else {
...
void storage_sync_sd_to_internal(void) {
    const char* targets[] = {"index.html", "users.html", "audit.html", "tester.html", "config.json", "network.json", "zones.json", "relays.json", "esphome.json"};
    for (int i = 0; i < 7; i++) {
        char src[64], dst[64];
        ...
        if (f_src == NULL) continue;
        
        FILE *f_dst = fopen(dst, "wb");
        if (f_dst) {
            char buf[512];
            size_t n;
            while ((n = fread(buf, 1, sizeof(buf), f_src)) > 0) fwrite(buf, 1, n, f_dst);
            fclose(f_dst);
...
void storage_init(void) {
    ...
    if (ret != ESP_OK) {
        ...
        ESP_LOGI(TAG, "SPIFFS Mounted: %d/%d KB used", (int)used/1024, (int)total/1024);
    }
...
void storage_add_rf_user(const char* name, const char* rf_code) {
    ...
    if (!root) root = cJSON_CreateArray(); // Create new if file doesn't exist
...
void storage_save_users(void) {
    ...
    for (int i = 0; i < u_count; i++) {
        ...
        // Save hashed PIN (new format)
        if (users[i].pin_hash[0] != '\0') {
            cJSON_AddStringToObject(item, "pin_hash", users[i].pin_hash);
        }
        // Keep legacy plaintext PIN for backward compatibility (but will be migrated)
        if (users[i].pin[0] != '\0') {
    ...
    if (json_str) {
        char path[128]; get_full_path(path, "users.json");
        FILE *f = fopen(path, "w");
...
void storage_save_config(void) {
    ...
    cJSON_AddStringToObject(root, "monitor_service_id", config.monitor_service_id);
    cJSON_AddStringToObject(root, "monitor_service_key", config.monitor_service_key);
    ...
    cJSON_AddBoolToObject(root, "is_telegram_enabled", config.is_telegram_enabled);
    ...
    char *rendered = cJSON_Print(root);
    if (rendered) {
        char path[128]; get_full_path(path, "config.json");
        FILE *f = fopen(path, "w");
        if (f) { fputs(rendered, f); fclose(f); }
        free(rendered);
    }
    cJSON_Delete(root);
}
...
void storage_load_all() {
    ...
    if ((data = read_file("config.json"))) {
        ...
        if (root) {
            ...
            if (cJSON_IsArray(nl_ids)) {
                for (int i = 0; i < cJSON_GetArraySize(nl_ids) && i < MAX_ALARM_SLOTS; i++) {
            ...
            config.is_monitor_medical = safe_get_bool(cJSON_GetObjectItem(root, "is_monitor_medical"));
    ...
    if ((data = read_file("users.json"))) {
        ...
        if (root) {
            cJSON_ArrayForEach(item, root) {
                ...
                memset(&users[u_count].password_hash, 0, sizeof(password_hash_t));
                
                // Load hashed PIN (new format - hex string for backwards compatibility)
                cJSON *pin_hash_item = cJSON_GetObjectItem(item, "pin_hash");
                if (pin_hash_item && cJSON_IsString(pin_hash_item)) {
                    strncpy(users[u_count].pin_hash, pin_hash_item->valuestring, sizeof(users[u_count].pin_hash) - 1);
                    users[u_count].pin_hash[sizeof(users[u_count].pin_hash) - 1] = '\0';
                    
                    // Try to parse hex string to binary password_hash (if it looks like hex)
                    if (strlen(users[u_count].pin_hash) > 32) {
                        // This is the old hex format, convert it
                        password_hash_from_hex(users[u_count].pin_hash, &users[u_count].password_hash);
                        users[u_count].use_legacy_pin = false;
                    } else {
                        users[u_count].use_legacy_pin = true;  // Will need migration
                    }
                } else {
                    users[u_count].pin_hash[0] = '\0';
                    users[u_count].use_legacy_pin = true;  // Will need migration
                }
                
                // Load legacy plaintext PIN (for migration)
                safe_strncpy(users[u_count].pin, cJSON_GetObjectItem(item, "pin"), STR_SMALL);
                
                // If we have a plaintext PIN and no hash, mark for migration
                if (strlen(users[u_count].pin) > 0 && users[u_count].use_legacy_pin) {
                    #ifdef ESP_PLATFORM
                    ESP_LOGW(TAG, "User '%s' loaded with plaintext PIN, needs migration", users[u_count].name);
                    #endif
                }
                ...
                if (emergency_pin_item && cJSON_IsString(emergency_pin_item)) {
                    strncpy(users[u_count].emergency_pin, emergency_pin_item->valuestring, sizeof(users[u_count].emergency_pin) - 1);
                    users[u_count].emergency_pin[sizeof(users[u_count].emergency_pin) - 1] = '\0';
                } else {
                ...
                users[u_count].requires_2fa = safe_get_bool(cJSON_GetObjectItem(item, "requires_2fa"));
                ...
                if (totp_secret_item && cJSON_IsString(totp_secret_item)) {
                    safe_strncpy(users[u_count].totp_secret, totp_secret_item, STR_MEDIUM);
                } else {
    ...
    if (u_count == 0 && config.pin[0] != '\0') {
        ...
        // Hash the master PIN using new binary format
        if (password_hash_create(config.pin, &users[0].password_hash)) {
            // Convert to hex for backwards compatibility with pin_hash field
            password_hash_to_hex(&users[0].password_hash, users[0].pin_hash);
            users[0].use_legacy_pin = false;  // Using new hash
            ...
            users[0].use_legacy_pin = true;  // Mark for migration
    ...
    if ((data = read_file("relays.json"))) {
        ...
        if (root) {
            cJSON_ArrayForEach(item, root) {
                ...
                relays[r_count].id = safe_get_int(cJSON_GetObjectItem(item, "id"), r_count);
    ...
    if ((data = read_file("zones.json"))) {
        ...
        if (root) {
            cJSON_ArrayForEach(item, root) {
                ...
                zones[z_count].id = safe_get_int(cJSON_GetObjectItem(item, "id"), z_count);
                ...
                safe_strncpy(zones[z_count].call, cJSON_GetObjectItem(item, "call"), STR_SMALL);
    ...
    if ((data = read_file("esphome.json"))) {
        ...
        if (root) {
            cJSON_ArrayForEach(item, root) {
                if (esphome_count >= 32) break;
                safe_strncpy(esphome_devices[esphome_count].hostname, cJSON_GetObjectItem(item, "hostname"), STR_SMALL);
                esphome_devices[esphome_count].port = safe_get_int(cJSON_GetObjectItem(item, "port"), 6053);
                safe_strncpy(esphome_devices[esphome_count].friendly_name, cJSON_GetObjectItem(item, "friendly_name"), STR_SMALL);
                safe_strncpy(esphome_devices[esphome_count].password, cJSON_GetObjectItem(item, "password"), STR_SMALL);
                safe_strncpy(esphome_devices[esphome_count].encryption_key, cJSON_GetObjectItem(item, "encryption_key"), STR_MEDIUM);
                esphome_devices[esphome_count].enabled = safe_get_bool(cJSON_GetObjectItem(item, "enabled"));
                
                // Parse nested zone object
                cJSON *zone_obj = cJSON_GetObjectItem(item, "zone");
                if (zone_obj) {
                    esphome_devices[esphome_count].virtual_zone_start = safe_get_int(cJSON_GetObjectItem(zone_obj, "id"), 0);
                    safe_strncpy(esphome_devices[esphome_count].description, cJSON_GetObjectItem(zone_obj, "description"), STR_MEDIUM);
                    safe_strncpy(esphome_devices[esphome_count].location, cJSON_GetObjectItem(zone_obj, "location"), STR_MEDIUM);
                } else {
                    // Fallback to old format
                    esphome_devices[esphome_count].virtual_zone_start = safe_get_int(cJSON_GetObjectItem(item, "virtual_zone_start"), 0);
                    safe_strncpy(esphome_devices[esphome_count].description, cJSON_GetObjectItem(item, "description"), STR_MEDIUM);
                    safe_strncpy(esphome_devices[esphome_count].location, cJSON_GetObjectItem(item, "location"), STR_MEDIUM);
                }
                
                // Parse nested relay object
                cJSON *relay_obj = cJSON_GetObjectItem(item, "relay");
                if (relay_obj) {
                    esphome_devices[esphome_count].virtual_relay_start = safe_get_int(cJSON_GetObjectItem(relay_obj, "id"), 0);
                    esphome_devices[esphome_count].entity_key = safe_get_uint32(cJSON_GetObjectItem(relay_obj, "entity_key"), 0);
                    printf("[STORAGE] ESPHome device %d: relay_id=%d, entity_key=%" PRIu32 "\n", 
                           esphome_count, esphome_devices[esphome_count].virtual_relay_start, 
                           esphome_devices[esphome_count].entity_key);
                } else {
                    // Fallback to old format
                    esphome_devices[esphome_count].virtual_relay_start = safe_get_int(cJSON_GetObjectItem(item, "virtual_relay_start"), 0);
                    esphome_devices[esphome_count].entity_key = 0; // Default for old format
                }
                    ...
                    cJSON_ArrayForEach(item, root) {
                        if (idx == i) {
                            ...
                            if (zone_obj) {
                                ...
                                safe_strncpy(vzone->description, cJSON_GetObjectItem(zone_obj, "description"), STR_MEDIUM);
                                safe_strncpy(vzone->type, cJSON_GetObjectItem(zone_obj, "type"), STR_SMALL);
                                safe_strncpy(vzone->call, cJSON_GetObjectItem(zone_obj, "call"), STR_SMALL);
                                safe_strncpy(vzone->location, cJSON_GetObjectItem(zone_obj, "location"), STR_SMALL);
                                safe_strncpy(vzone->model, cJSON_GetObjectItem(zone_obj, "model"), STR_SMALL);
                                safe_strncpy(vzone->manufacturer, cJSON_GetObjectItem(zone_obj, "manufacturer"), STR_SMALL);
                                vzone->is_chime = safe_get_bool(cJSON_GetObjectItem(zone_obj, "is_chime"));
                                vzone->is_alarm_on_armed_only = safe_get_bool(cJSON_GetObjectItem(zone_obj, "is_alarm_on_armed_only"));
                                vzone->gpio = -1;  // Virtual - no GPIO
                                vzone->is_i2c = false;
                                vzone->i2c_address = 0;
                                vzone->is_perimeter = safe_get_bool(cJSON_GetObjectItem(zone_obj, "is_perimeter"));
                                vzone->is_interior = safe_get_bool(cJSON_GetObjectItem(zone_obj, "is_interior"));
                                vzone->is_panic = safe_get_bool(cJSON_GetObjectItem(zone_obj, "is_panic"));
                    ...
                    cJSON_ArrayForEach(item, root) {
                        if (idx == i) {
                            ...
                            if (relay_obj) {
                                ...
                                safe_strncpy(vrelay->description, cJSON_GetObjectItem(relay_obj, "description"), STR_MEDIUM);
                                vrelay->duration = safe_get_int(cJSON_GetObjectItem(relay_obj, "duration"), 0);
                                safe_strncpy(vrelay->location, cJSON_GetObjectItem(relay_obj, "location"), STR_MEDIUM);
                                safe_strncpy(vrelay->type, cJSON_GetObjectItem(relay_obj, "type"), STR_SMALL);
                                vrelay->is_repeat = safe_get_bool(cJSON_GetObjectItem(relay_obj, "is_repeat"));
...
void storage_load_network(void) {
    ...
    if (root) {
        ...
        safe_strncpy(net_cfg.ip, cJSON_GetObjectItem(root, "ip"), sizeof(net_cfg.ip));
        safe_strncpy(net_cfg.netmask, cJSON_GetObjectItem(root, "netmask"), sizeof(net_cfg.netmask));
        safe_strncpy(net_cfg.gateway, cJSON_GetObjectItem(root, "gateway"), sizeof(net_cfg.gateway));
        safe_strncpy(net_cfg.dns, cJSON_GetObjectItem(root, "dns"), sizeof(net_cfg.dns));
...
char *esphome_devices_to_json(void) {
    ...
    for (int i = 0; i < esphome_count; i++) {
        ...
        cJSON_AddStringToObject(dev, "friendly_name", esphome_devices[i].friendly_name);
        cJSON_AddStringToObject(dev, "password", esphome_devices[i].password);
        cJSON_AddStringToObject(dev, "encryption_key", esphome_devices[i].encryption_key);
        cJSON_AddNumberToObject(dev, "virtual_zone_start", esphome_devices[i].virtual_zone_start);
        cJSON_AddNumberToObject(dev, "virtual_relay_start", esphome_devices[i].virtual_relay_start);
...
int esphome_device_add(const char *hostname, uint16_t port, const char *friendly_name,
                       const char *password, const char *encryption_key,
                       int8_t virtual_zone_start, int8_t virtual_relay_start, bool enabled) {
    ...
    strncpy(dev->encryption_key, encryption_key ? encryption_key : "", STR_MEDIUM - 1);
...
int esphome_device_update(const char *hostname, uint16_t port, const char *friendly_name,
                          const char *password, const char *encryption_key,
                          int8_t virtual_zone_start, int8_t virtual_relay_start, bool enabled) {
    if (!hostname) {
        return -1;
    ...
    for (int i = 0; i < esphome_count; i++) {
        if (strcmp(esphome_devices[i].hostname, hostname) == 0) {
            if (port > 0) esphome_devices[i].port = port;
            if (friendly_name) {
            ...
            if (encryption_key) {
                strncpy(esphome_devices[i].encryption_key, encryption_key, STR_MEDIUM - 1);
                esphome_devices[i].encryption_key[STR_MEDIUM - 1] = '\0';
            }
            if (virtual_zone_start >= 0) esphome_devices[i].virtual_zone_start = virtual_zone_start;
            if (virtual_relay_start >= 0) esphome_devices[i].virtual_relay_start = virtual_relay_start;
...
/**
 * @brief Migrate a single user from plaintext PIN to hashed password
 * @param user_index Index of user in users[] array
 * @return ESP_OK on success, ESP_FAIL if user doesn't exist or already migrated
 */
esp_err_t user_migrate_to_hash(int user_index) {
    ...
    if (!user->use_legacy_pin) {
        #ifdef ESP_PLATFORM
        ESP_LOGW(TAG, "User '%s' already using hashed password", user->name);
        #endif
        return ESP_OK;
    }
    ...
    if (strlen(user->pin) == 0) {
        #ifdef ESP_PLATFORM
        ESP_LOGE(TAG, "User '%s' has no PIN to migrate", user->name);
        #endif
        return ESP_FAIL;
    }
    
    // Create hash from plaintext PIN
    if (!password_hash_create(user->pin, &user->password_hash)) {
        #ifdef ESP_PLATFORM
        ESP_LOGE(TAG, "Failed to create password hash for user '%s'", user->name);
        #endif
        return ESP_FAIL;
    }
    
    // Clear plaintext PIN and mark as migrated
    memset(user->pin, 0, sizeof(user->pin));
    user->use_legacy_pin = false;
    
    #ifdef ESP_PLATFORM
    ESP_LOGI(TAG, "Migrated user '%s' to hashed password", user->name);
    #endif
...
bool user_authenticate_pin(const char *pin, user_t **out_user) {
    ...
    for (int i = 0; i < u_count; i++) {
        ...
        if (user->use_legacy_pin) {
            // Legacy: compare plaintext PIN
            authenticated = (strcmp(user->pin, pin) == 0);
            
            #ifdef ESP_PLATFORM
            if (authenticated) {
                ESP_LOGW(TAG, "User '%s' authenticated with legacy PIN (consider migrating)", user->name);
            }
            #endif
        } else {
            // New: verify hashed password
            authenticated = password_hash_verify(pin, &user->password_hash);
            
            #ifdef ESP_PLATFORM
            if (authenticated) {
                ESP_LOGI(TAG, "User '%s' authenticated with hashed password", user->name);
            }
            #endif
        }
...
esp_err_t user_set_password(int user_index, const char *new_pin) {
    ...
    // Create new hash
    if (!password_hash_create(new_pin, &user->password_hash)) {
        #ifdef ESP_PLATFORM
        ESP_LOGE(TAG, "Failed to create password hash for user '%s'", user->name);
        #endif
        return ESP_FAIL;
    ...
    // Clear legacy PIN and mark as using hash
    memset(user->pin, 0, sizeof(user->pin));
    user->use_legacy_pin = false;
...
/**
 * @brief Migrate all users with plaintext PINs to hashed passwords
 * @return ESP_OK if all users migrated successfully, ESP_FAIL otherwise
 */
esp_err_t users_migrate_all_to_hash(void) {
    ...
    for (int i = 0; i < u_count; i++) {
        ...
        // Attempt migration
        if (user_migrate_to_hash(i) == ESP_OK) {
            migrated_count++;
        } else {
            failed_count++;
        }
    ...
    #ifdef ESP_PLATFORM
    if (migrated_count > 0) {
        ESP_LOGI(TAG, "Migrated %d user(s) to hashed passwords", migrated_count);
    }
    if (failed_count > 0) {
        ESP_LOGE(TAG, "Failed to migrate %d user(s)", failed_count);
    }
    if (migrated_count == 0 && failed_count == 0) {
        ESP_LOGI(TAG, "No users needed migration");
    }
    #endif
