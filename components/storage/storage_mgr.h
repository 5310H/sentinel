...
#include "../esphome_api/esphome_api_protocol.h"
...
/**
 * @brief Noise protocol handshake state
#include "esphome_api/esphome_api_protocol.h"  // For noise_handshake_t

// Legacy noise handshake structure (deprecated - use noise_handshake_t from esphome_api_protocol.h)
typedef struct {
    unsigned char e[32];        // Client ephemeral public key
    unsigned char s[32];        // Client static secret key
    unsigned char psk[32];      // Pre-shared key
    unsigned char ck[32];       // Chaining key
    unsigned char h[32];        // Handshake hash
    unsigned char k[32];        // Encryption key
    unsigned char n[12];        // Nonce
    int state;                  // Handshake state
} noise_handshake_legacy_t;

// Noise handshake states
#define NOISE_STATE_INIT      0
#define NOISE_STATE_SENT_E    1
#define NOISE_STATE_COMPLETE  2
...
typedef struct {
    char name[STR_SMALL];
    char pin[STR_SMALL];              // DEPRECATED: Legacy plaintext PIN (for migration only)
    char pin_hash[136 + 1];           // NEW: Hex-encoded password hash (salt+hash+iterations)
    password_hash_t password_hash;    // NEW: Binary password hash structure
    bool use_legacy_pin;              // NEW: Flag for migration (true = still using plaintext)
    char emergency_pin[STR_SMALL];    // Plaintext PIN for emergency services (Noonlight)
    char phone[STR_SMALL];
    char email[STR_MEDIUM];
    int notify;
    bool is_admin;
    bool requires_2fa;
    char totp_secret[STR_MEDIUM];  // Base32-encoded TOTP secret (e.g., "JBSWY3DPEHPK3PXP")
...
typedef struct {
    ...
    noise_handshake_t handshake;       // Noise handshake state for encryption
} esphome_device_t;
...
typedef struct {
    ...
    double latitude; double longitude; int accuracy;
    ...
    bool is_monitor_fire; bool is_monitor_police;
    bool is_monitor_medical; bool is_monitor_other;
    char smtp_server[STR_MEDIUM]; int smtp_port;
    char smtp_user[STR_MEDIUM]; char smtp_pass[STR_MEDIUM];
    char mqtt_server[STR_MEDIUM]; int mqtt_port;
    char mqtt_user[STR_SMALL]; char mqtt_pass[STR_SMALL];
    char telegram_id[STR_SMALL]; char telegram_token[STR_MEDIUM];
    bool is_telegram_enabled;
    char nvrserver_url[STR_MEDIUM]; char haintegration_url[STR_MEDIUM];
    int entry_delay; int exit_delay; int cancel_delay;
} config_t;
...
extern esphome_device_t esphome_devices[32];  // Max 32 ESPHome devices
extern int z_count, r_count, u_count, esphome_count;
...
/**
 * @brief Load all configu// A helper to dump the memory map into a string instead of the console
void get_storage_debug_string(char *dest, size_t max_len) {
    int offset = 0;
    offset += snprintf(dest + offset, max_len - offset, "--- HARDWARE STORAGE MAP ---\n");
    offset += snprintf(dest + offset, max_len - offset, "MASTER_PIN: [%s]\n", config.pin);
    offset += snprintf(dest + offset, max_len - offset, "USER_COUNT: %d\n", u_count);
 ...
 * 
 * Finds device by hostname and updates fields. Pass NULL/0/-1 to leave fields unchanged.
 * 
...
/**
 * @brief Migrate user from plaintext PIN to hashed password
 * 
...
/**
 * @brief Migrate all users to hashed passwords
 * 
 * @return esp_err_t ESP_OK if all migrations successful
 */
