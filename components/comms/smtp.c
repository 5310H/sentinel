...
static void smtp_execute_send(config_t *config, const char *target_email, const char *subject, const char *body) {
    ...
    int ret = esp_tls_conn_new_sync(config->smtp_server, 
                                   strlen(config->smtp_server), 
                                   config->smtp_port, 
                                   &cfg, 
                                   tls);
    ...
    if (ret == 1) { // Success
        ...
        // 1. Read Greeting
        if ((len = esp_tls_conn_read(tls, buf, sizeof(buf)-1)) < 0) goto smtp_err;

        // 2. EHLO
        snprintf(buf, sizeof(buf), "EHLO sentinel\r\n");
        if (esp_tls_conn_write(tls, buf, strlen(buf)) < 0) goto smtp_err;
        if ((len = esp_tls_conn_read(tls, buf, sizeof(buf)-1)) < 0) goto smtp_err;
        ...
        // 4. Username
        mbedtls_base64_encode((unsigned char*)b64, sizeof(b64), &n, (const unsigned char*)config->smtp_user, strlen(config->smtp_user));
        b64[n] = 0;
        ...
        // 5. Password
        mbedtls_base64_encode((unsigned char*)b64, sizeof(b64), &n, (const unsigned char*)config->smtp_pass, strlen(config->smtp_pass));
        b64[n] = 0;
        snprintf(buf, sizeof(buf), "%s\r\n", b64);
        if (esp_tls_conn_write(tls, buf, strlen(buf)) < 0) goto smtp_err;
        if ((len = esp_tls_conn_read(tls, buf, sizeof(buf)-1)) < 0) goto smtp_err;
    ...
    // Note: fmemopen is great for Linux, but we avoid it on ESP32 to stay portable
...
void smtp_alert_all_contacts(config_t *config, user_t *users, int user_count, zone_t *z) {
    ...
    char subject[256];
    char body[512];
    snprintf(subject, sizeof(subject), "CRITICAL: Sentinel Alarm - %s by %s", z->name, z->description);

    // STEP 1: CATEGORY SELECTION
    if (is_type(z->call, "fire")) {
       snprintf(body, sizeof(body), "Active FIRE detected in %s by %s. Please notify local FIRE authorities.", z->name, z->description);
    } 
    else if (is_type(z->call, "police")) {
       snprintf(body, sizeof(body), "Security breach detected in %s by %s. Please notify local police authorities.", z->name, z->description);  
    } 
...
void smtp_send_cancellation(config_t *config, user_t *users, int user_count) {
    ...
    for (int i = 0; i < user_count; i++) {
        if (users[i].notify == 2 && users[i].email[0] != '\0') {
            smtp_execute_send(config, users[i].email, subject, body);
        }
    }
}