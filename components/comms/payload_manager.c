...
void payload_build_noonlight(config_t *o, zone_t *z, bool is_update) {
    if (!is_update) {
        // Note: Check if your header uses 'address1' or 'Address1' (C is case sensitive)
...
// CHANGE: struct config* -> config_t* |  struct zone* -> zone_t* | struct contact* -> user_t*
...
bool payload_parse_response(int slot, const char *raw, char *out_id) {
    ...
    if (slot == SLOT_NOONLIGHT) {
        ...
        if (p) {
            sscanf(p + 6, "%63[^\"]", out_id);  // Limit to 63 chars (assumes 64-byte buffer)
            return true;
        }
