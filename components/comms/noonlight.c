...
#endif
static bool do_request(config_t *conf, const char *url, const char *method, const char *data, char *out_buffer, size_t out_len) {
#ifdef ESP_PLATFORM
    ...
    snprintf(auth_header, sizeof(auth_header), "Bearer %s", conf->monitor_service_key);
    esp_http_client_set_header(client, "Content-Type", "application/json");
    esp_http_client_set_header(client, "Authorization", auth_header);
    if (data) esp_http_client_set_post_field(client, data, strlen(data));
    ...
    if (err == ESP_OK) {
        ...
        if (status >= 200 && status < 300) {
            if (out_buffer) {
                int read = esp_http_client_read_response(client, out_buffer, out_len - 1);
                if (read >= 0) out_buffer[read] = 0;
            }
            success = true;
        }
    }
    ...
    if (curl) {
        ...
        snprintf(auth, sizeof(auth), "Authorization: Bearer %s", conf->monitor_service_key);
        headers = curl_slist_append(headers, auth);
        curl_easy_setopt(curl, CURLOPT_URL, url);
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
        if (strcmp(method, "PATCH") == 0) curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "PATCH");
...
bool noonlight_create_alarm(config_t *conf, zone_t *z, int slot) {
    ...
    if (do_request(conf, conf->monitoring_url, "POST", data, response_buffer, 2048)) {
...
void noonlight_log_event(config_t *conf, zone_t *z, const char* alarm_id) {
    if (!alarm_id || strlen(alarm_id) == 0) return;
    char url[256];
...
void noonlight_send_instructions(config_t *conf, const char* alarm_id) {
    if (!alarm_id || strlen(conf->instructions) == 0) return;
    char url[256];
    snprintf(url, sizeof(url), "%s/%s/instructions", conf->monitoring_url, alarm_id);
    
    cJSON *root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "instruction", conf->instructions);
...
void noonlight_sync_people(config_t *conf, user_t *users_list, int count, const char* alarm_id) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        cJSON_AddStringToObject(root, "pin", users_list[i].pin);
...
bool noonlight_cancel_alarm(config_t *conf, const char *id) {
    ...
    cJSON *root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "status", "canceled");
    cJSON_AddStringToObject(root, "pin", conf->pin);
    
    char *data = cJSON_PrintUnformatted(root);
    bool res = do_request(conf, url, "PATCH", data, NULL, 0);
    free(data);
    cJSON_Delete(root);
    return res;
}
...
int os_get_noonlight_status(const char* alarm_id) {
    if (!alarm_id || strlen(alarm_id) == 0) return 0;
    char response_buffer[2048] = {0};
    char url[256];
    int result_status = 1; 
    ...
    if (do_request(&config, url, "GET", NULL, response_buffer, 2048)) {
        ...
        if (root) {
            ...
            if (st && st->valuestring && strcmp(st->valuestring, "CANCELED") == 0) result_status = 2;
            cJSON_Delete(root);
        }
    }
...
void noonlight_sync_task(config_t *conf) {
    if (!conf || conf->notify != 4) return;
    for (int i = 0; i < MAX_ALARM_SLOTS; i++) {
        if (strlen(conf->nl_ids[i]) > 0) {
            if (os_get_noonlight_status(conf->nl_ids[i]) == 2) {
