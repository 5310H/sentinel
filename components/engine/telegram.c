...
static esp_err_t telegram_http_event_handler(esp_http_client_event_t *evt) {
    switch(evt->event_id) {
        ...
        case HTTP_EVENT_ON_HEADER:
            LOG_DEBUG(TAG, "HTTP Response Header: %.*s", evt->data_len, (char *)evt->data);
            break;
...
static void telegram_raw_send_async(const char* token, const char* chat_id, const char* text) {
    // Validate all inputs
    if (token == NULL || token[0] == '\0' || 
        chat_id == NULL || chat_id[0] == '\0' || 
        text == NULL || text[0] == '\0') {
        return;
    }

    if (strlen(token) < 5 || strlen(chat_id) < 3 || strlen(text) < 1) return;

    // Build JSON payload (handles escaping of special chars/newlines automatically)
...
static size_t telegram_curl_callback(void *contents, size_t size, size_t nmemb, void *userp) {
    size_t realsize = size * nmemb;
    printf("[%s] Telegram API Response: %.*s\n", TAG, (int)realsize, (char*)contents);
    (void)userp;
    return realsize;
}
...
void telegram_send_alert(config_t *conf, zone_t *z) {
    if (conf == NULL || z == NULL || z->name[0] == '\0') return;
    ...
    // Safely truncate description to prevent buffer overflow
    char desc_truncated[STR_MEDIUM];
    strncpy(desc_truncated, z->description, sizeof(desc_truncated) - 1);
    desc_truncated[sizeof(desc_truncated) - 1] = '\0';
    ...
    snprintf(msg, sizeof(msg), 
        ...
        z->name, z->call, desc_truncated, time_str);

    telegram_raw_send_async(conf->telegram_token, conf->telegram_id, msg);
}

void telegram_send_cancellation(config_t *conf) {
    if (conf == NULL || conf->name[0] == '\0') return;
