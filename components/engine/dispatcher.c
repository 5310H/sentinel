...
void dispatcher_alert(zone_t *z) {
  ...
  int notify_val = storage_get_config()->notify;
  storage_unlock();
  ...
  LOG_WARN(TAG, "Dispatching alert for %s (Method: %d)", z->name, notify_val);
  ...
  case 4: // NOONLIGHT (Professional Monitoring)
  {
    ...
    if (zone_call == ZONE_CALL_FIRE)
      slot = SLOT_FIRE; // Slot 0
    else if (zone_call == ZONE_CALL_POLICE)
      slot = SLOT_POLICE; // Slot 1
    else if (zone_call == ZONE_CALL_MEDICAL)
      slot = SLOT_MED; // Slot 2

    // Only trigger if a dispatch isn't already active in this slot
    storage_lock();
    bool slot_active = (strlen(storage_get_config()->nl_ids[slot]) > 0);
    storage_unlock();

    if (!slot_active) {
      noonlight_create_alarm(storage_get_config(), z, slot);
    } else {
      LOG_DEBUG(TAG, "Noonlight slot %d already active. Skipping duplicate.",
                slot);
    }
  } break;
...
void dispatcher_cancel_alert(void) {
  ...
  char ids[4][64] = {0};
  for (int i = 0; i < 4; i++) {
    strncpy(ids[i], storage_get_config()->nl_ids[i], 63);
  }
  storage_unlock();

  if (notify_val == 4) {
    // Attempt to cancel all 4 possible Noonlight slots (Fire, Police, Med,
    // Other)
    for (int i = 0; i < MAX_ALARM_SLOTS && i < 4; i++) {
      if (ids[i][0] != '\0' && strlen(ids[i]) > 0) {
        noonlight_cancel_alarm(storage_get_config(), ids[i]);
        // Note: ID is cleared by noonlight.c/sync_task once confirmed by cloud
      }
    }
  } else if (notify_val == 3) {
    telegram_send_cancellation(storage_get_config());
  } else if (notify_val == 2) {
