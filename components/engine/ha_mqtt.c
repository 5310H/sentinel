...
static int _publish_alarm_discovery(void) {
  ...
  if (modes) {
    cJSON *s1 = cJSON_CreateString("disarmed");
    if (s1)
      cJSON_AddItemToArray(modes, s1);
    cJSON *s2 = cJSON_CreateString("armed_away");
    if (s2)
      cJSON_AddItemToArray(modes, s2);
    cJSON *s3 = cJSON_CreateString("armed_home");
    if (s3)
      cJSON_AddItemToArray(modes, s3);
    cJSON *s4 = cJSON_CreateString("armed_night");
    if (s4)
      cJSON_AddItemToArray(modes, s4);
    cJSON *s5 = cJSON_CreateString("triggered");
    if (s5)
      cJSON_AddItemToArray(modes, s5);
  ...
  cJSON_AddItemToObject(config, "payload_arm_night",
                        cJSON_CreateString("night"));

  cJSON *formats = cJSON_CreateArray();
  if (formats) {
    cJSON *s6 = cJSON_CreateString("number");
    if (s6)
      cJSON_AddItemToArray(formats, s6);
    cJSON_AddItemToObject(config, "supported_code_formats", formats);
  }
  ...
  int ret =
      _mqtt_pub("homeassistant/alarm_control_panel/sentinel/config", payload);
  free(payload);
  cJSON_Delete(config);
...
static int _publish_tamper_discovery(void) {
  ...
  char *payload = cJSON_Print(config);
  int ret =
      _mqtt_pub("homeassistant/binary_sensor/sentinel_tamper/config", payload);
  free(payload);
  cJSON_Delete(config);
...
static int _publish_power_discovery(void) {
  ...
  char *payload = cJSON_Print(config);
  int ret =
      _mqtt_pub("homeassistant/binary_sensor/sentinel_power/config", payload);
  free(payload);
  cJSON_Delete(config);
...
static int _publish_relays_discovery(void) {
  for (int i = 0; i < storage_get_relay_count() && i < 8; i++) {
    ...
    snprintf(discovery_topic, sizeof(discovery_topic),
             "homeassistant/switch/sentinel_relay_%d/config", i);
...
int ha_mqtt_publish_state(void) {
  ...
  snprintf(temp_str, sizeof(temp_str), "%d",
           (int)((status.temperature_c * 9 / 5) + 32));
  ...
  for (int i = 0; i < storage_get_relay_count() && i < 8; i++) {
    ...
    storage_lock();
    int gpio = storage_get_relay(i)->gpio;
    storage_unlock();
    int relay_state = hal_get_relay_state(gpio);
  ...
  for (int i = 0; i < storage_get_zone_count() && i < 16; i++) {
    ...
    storage_unlock();
    int zone_state = hal_get_zone_state(gpio);
...
int ha_mqtt_publish_relay_state(uint8_t relay_index, bool state) {
  if (relay_index >= storage_get_relay_count() || relay_index >= 8)
    return -1;
...
int ha_mqtt_publish_tamper(bool detected) {
  return _mqtt_pub("sentinel/tamper", detected ? "detected" : "safe") == 0 ? 0
                                                                           : -1;
}

int ha_mqtt_publish_power(bool on_backup) {
  return _mqtt_pub("sentinel/power", on_backup ? "backup" : "normal") == 0 ? 0
                                                                           : -1;
}

int ha_mqtt_publish_zone_state(uint8_t zone_index, bool open) {
  if (zone_index >= storage_get_zone_count() || zone_index >= 16)
    return -1;
...
int ha_mqtt_handle_command(const char *topic, const uint8_t *payload,
  ...
  if (strstr(topic, "sentinel/relay/") && strstr(topic, "/command")) {
    ...
    if (relay_idx >= 0 && relay_idx < storage_get_relay_count()) {
      ...
      storage_lock();
      int gpio = storage_get_relay(relay_idx)->gpio;
      storage_unlock();
      hal_set_relay(gpio, on);
