...
bool noonlight_cancel_alarm(config_t *conf, const char *id) {
    ...
    cJSON *root = cJSON_CreateObject();
    cJSON_AddStringToObject(root, "status", "CANCELED");  // FIX: Must be uppercase per API spec
    cJSON_AddStringToObject(root, "pin", conf->pin);
    
    char *data = cJSON_PrintUnformatted(root);
    bool res = do_request(conf, url, "POST", data, NULL, 0);  // FIX: API requires POST, not PATCH
    free(data);
    cJSON_Delete(root);
    return res;
}
