    ...
    switch (id) {
        case ETHERNET_EVENT_CONNECTED:
            ...
            if (g_eth_reconnect_pending) {
                uint64_t downtime_ms = (esp_timer_get_time() - g_eth_disconnect_time) / 1000;
...
#if W5500_LOG_RECONNECT
            ESP_LOGW(TAG, "Ethernet Link Down - will attempt restart in %lu ms", ETH_RECONNECT_TIMEOUT_MS);
#else
...
#if W5500_VERBOSE_DEBUG
    ESP_LOGI(TAG, "W5500 SPI config: clock=%dMHz, queue=%d, CS=%d, INT=%d, RST=%d",
             W5500_SPI_CLOCK_MHZ, W5500_SPI_QUEUE_SIZE, W5500_CS, W5500_INT, W5500_RST);
#endif
    ...
    for (;;) {
        ...
        if (g_eth_reconnect_pending) {
            ...
            if (disconnect_duration > (ETH_RECONNECT_TIMEOUT_MS * 1000)) {
                ...
                // Note: Full restart would require passing eth_handle from init_ethernet_v3
                // For now, log the attempt and rely on hardware to recover
                g_eth_reconnect_pending = false;
            }
        }
#endif
