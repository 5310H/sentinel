...
// Integration globals
// --- 1. PROTOTYPES & EXTERNS ---
extern config_t config;  // This pulls in the global config object
extern user_t users[];   // This pulls in the users array
extern int u_count;      // This pulls in the user count
extern network_t net_cfg;
extern zone_t zones[];
extern int z_count;
...
keypad_result_t __wrap_engine_check_keypad(const char *pin) {
    ...
    if (config.pin[0] != '\0') {
        if (strcmp(pin, config.pin) == 0) {
            ...
            res.is_admin = true;
            strncpy(res.name, config.name, sizeof(res.name) - 1);
    ...
    if (user_authenticate_pin(pin, &user) && user != NULL) {
        printf("RESULT: MATCHED USER [%s] (%s)\n", user->name, 
               user->use_legacy_pin ? "plaintext PIN - legacy" : "hashed PIN");
        res.authenticated = true;
...
bool __wrap_hal_get_relay_state(int gpio) { 
    ...
    return (relay_idx >= 0 && relay_idx < MAX_RELAYS) ? mock_relay_states[relay_idx] : false; 
}
void __wrap_hal_set_siren(bool active) { printf("\n[HAL] SIREN -> %s\n", active ? "ON" : "OFF"); }
void __wrap_hal_set_strobe(bool active) { printf("\n[HAL] STROBE -> %s\n", active ? "ON" : "OFF"); }
...
void esphome_reject_handler(struct mg_connection *c, int ev, void *ev_data) {
    ...
    if (ev == MG_EV_ACCEPT) {
        ...
        printf("[WARNING] ESPHome connection attempt from %s (device misconfigured - should connect TO us, not FROM us)\n", 
               c->rem.is_ip6 ? "[IPv6]" : "[IPv4]");
    }
    else if (ev == MG_EV_READ && c->recv.len > 0) {
        // Data received - reject the connection
        printf("[WARNING] Rejecting ESPHome protocol data from misconfigured device\n");
        c->is_closing = 1;
    }
...
void web_handler(struct mg_connection *c, int ev, void *ev_data) {
    ...
    if (ev == MG_EV_ACCEPT && c->fn_data != NULL) {
        ...
        struct mg_tls_opts opts = {
            .cert = mg_str("./server.pem"),  // Certificate file path for Linux testing
            .key = mg_str("./server.pem"),   // Key file (same file contains both cert and key)
        };
        mg_tls_init(c, &opts);
    } 
    // Handle ESPHome protocol connections (reject them gracefully)
        ...
        // ESPHome devices may be misconfigured to connect to our server as their API endpoint,
        // but we should be connecting TO them, not receiving connections FROM them.
        // Reject non-HTTP connections to prevent socket errors.
        if (c->recv.len > 0) {
            char *data = (char*)c->recv.buf;
            // Check if the first few bytes look like HTTP methods
            if (c->recv.len >= 3 && 
                strncmp(data, "GET", 3) != 0 && 
                ...
                printf("[WARNING] Rejecting non-HTTP connection (likely ESPHome protocol - device should connect TO us, not FROM us)\n");
        ...
        if (mg_strcmp(hm->uri, mg_str("/api/users")) == 0) {
            ...
            get_json_str(hm->body, "$.is_admin", is_admin_buf, sizeof(is_admin_buf));
            bool is_admin = (strcmp(is_admin_buf, "true") == 0);
            user_add(users, &u_count, name, pin, "", email, atoi(n_buf), is_admin, emergency_pin); 
            mg_http_reply(c, 200, "Content-Type: application/json\r\n", "{\"status\":\"ok\"}");
        }
            ...
            get_json_str(hm->body, "$.emergency_pin", emergency_pin, sizeof(emergency_pin));
            get_json_str(hm->body, "$.is_admin", is_admin_buf, sizeof(is_admin_buf));
            int is_admin = (strcmp(is_admin_buf, "true") == 0) ? 1 : 0;
            user_update(users, u_count, name, pin, "", email, atoi(n_buf), is_admin, emergency_pin);
            mg_http_reply(c, 200, "Content-Type: application/json\r\n", "{\"status\":\"ok\"}");
        }
            ...
            if (strlen(name) > 0 && strlen(totp_secret) > 0) {
                ...
                // Check if this is the config.pin user (name matches config.name)
        ...
        // 2. AUTH & CONTROL (2FA Support)
            ...
            if (strcmp(step_buf, "pin") == 0) {
                if (res.authenticated) {
                    ...
                    if (!res.is_admin || strcmp(res.name, config.name) != 0) {
        ...
        // 3. API: Full Status (Matches index.html and maintenance.html expectations)
            ...
            // Console Debugging
//            printf("[DEBUG] Web UI Status Request: %d Zones, %d Relays detected\n", z_count, r_count);
            ...
            int off = snprintf(s_buf, sizeof(s_buf), 
                ...
                "\"violation\":\"%s\",\"violation_type\":\"%s\",\"uptime\":\"%s\",\"mac\":\"%s\"},", 
                ...
                config.monitor_service_key, config.monitoring_url, config.notify,
                ...
                up_str, "AC:DC:12:34:56:78");

            off += snprintf(s_buf + off, sizeof(s_buf) - off, 
                "\"network\":{\"use_dhcp\":%s,\"ip\":\"%s\",\"netmask\":\"%s\",\"gateway\":\"%s\",\"dns\":\"%s\"},",
                net_cfg.use_dhcp ? "true" : "false", net_cfg.ip, net_cfg.netmask, net_cfg.gateway, net_cfg.dns);
            off += snprintf(s_buf + off, sizeof(s_buf) - off, "\"rf\":{\"last\":\"%s\"},", mock_last_rf);
            ...
            for (int i = 0; i < z_count; i++) {
                if (zones[i].gpio >= 0) {  // Only physical zones with GPIO
                    // In digitalRead, 0 usually means "Closed/Short" (Violated for NC sensors)
                    bool is_open = (digitalRead(zones[i].gpio) == 0);
                    off += snprintf(s_buf + off, sizeof(s_buf) - off, 
                        "{\"id\":%d,\"name\":\"%s\",\"description\":\"%s\",\"location\":\"%s\",\"open\":%s,\"violated\":%s},",
                        zones[i].id, zones[i].name, zones[i].description, zones[i].location,
                        is_open ? "true" : "false",
                        is_open ? "true" : "false");
                }
            }
            ...
            for (int i = 0; i < esphome_count; i++) {
                if (esphome_devices[i].virtual_zone_start > 0 && esphome_devices[i].enabled) {
                    int zone_id = esphome_devices[i].virtual_zone_start;
                    const char *desc = esphome_devices[i].description[0] != '\0' ? esphome_devices[i].description : esphome_devices[i].friendly_name;
                    off += snprintf(s_buf + off, sizeof(s_buf) - off,
                        "{\"id\":%d,\"name\":\"%s\",\"description\":\"%s\",\"location\":\"%s\",\"open\":false,\"violated\":false},",
                        zone_id, esphome_devices[i].friendly_name, desc, esphome_devices[i].location);
                }
            }
            ...
            if (tuya_mgr_get_devices(tuya_devs, TUYA_DEVICE_MAX_COUNT, &tuya_count) == ESP_OK) {
                for (int i = 0; i < tuya_count; i++) {
                    if (tuya_devs[i].virtual_zone_id > 0 && tuya_devs[i].enabled) {
                        ...
                        const char *desc = tuya_devs[i].description[0] != '\0' ? tuya_devs[i].description : tuya_devs[i].name;
                        off += snprintf(s_buf + off, sizeof(s_buf) - off,
                            "{\"id\":%d,\"name\":\"%s\",\"description\":\"%s\",\"location\":\"%s\",\"open\":%s,\"violated\":%s},",
                            tuya_devs[i].virtual_zone_id, tuya_devs[i].name, desc, tuya_devs[i].location,
                            is_open ? "true" : "false",
                            is_open ? "true" : "false");
                    }
                }
            ...
            for (int i = 0; i < r_count; i++) {
                if (relays[i].id <= 8) {  // Only physical relays
                    off += snprintf(s_buf + off, sizeof(s_buf) - off, 
                        ...
                        relays[i].id, relays[i].name, relays[i].location,
            ...
            for (int i = 0; i < esphome_count; i++) {
                if (esphome_devices[i].virtual_relay_start > 0 && esphome_devices[i].enabled) {
                    int relay_id = esphome_devices[i].virtual_relay_start;
                    int esphome_relay_idx = relay_id - 8;
                    bool active = (esphome_relay_idx >= 0 && esphome_relay_idx < 24) ? 
                        mock_esphome_relay_states[esphome_relay_idx] : false;
                    off += snprintf(s_buf + off, sizeof(s_buf) - off,
                        "{\"id\":%d,\"name\":\"%s\",\"location\":\"%s\",\"active\":%s},",
                        relay_id, esphome_devices[i].friendly_name, esphome_devices[i].location,
                        active ? "true" : "false");
                }
            }
            ...
            if (tuya_mgr_get_devices(tuya_relay_devs, TUYA_DEVICE_MAX_COUNT, &tuya_relay_count) == ESP_OK) {
                for (int i = 0; i < tuya_relay_count; i++) {
                    if (tuya_relay_devs[i].virtual_relay_id > 0 && tuya_relay_devs[i].enabled) {
                        off += snprintf(s_buf + off, sizeof(s_buf) - off,
                            "{\"id\":%d,\"name\":\"%s\",\"location\":\"%s\",\"active\":%s},",
                            tuya_relay_devs[i].virtual_relay_id, tuya_relay_devs[i].name, tuya_relay_devs[i].location,
            ...
            off += snprintf(s_buf + off, sizeof(s_buf) - off, "]}"); // Final Closing Brace
        ...
        // 4. API: System Status (for index.html temperature/power/tamper display)
            ...
            snprintf(status_buf, sizeof(status_buf),
                "{\"temperature_c\":%d,\"temperature_f\":%d,\"on_backup_power\":%s,\"tamper_detected\":%s}",
                status.temperature_c, temp_f, status.on_backup_power ? "true" : "false", 
       ...
       // 7. API: Zones (for zstatus.html)
            ...
            for (int i = 0; i < z_count; i++) {
                off += snprintf(z_buf + off, 16384 - off, 
                    ...
                    zones[i].id, zones[i].description, zones[i].location, (digitalRead(zones[i].gpio) == 0) ? "true" : "false");
            }
            ...
            for (int i = 0; i < esphome_count; i++) {
                if (esphome_devices[i].enabled) {
                    off += snprintf(z_buf + off, 16384 - off,
                        "{\"id\":%d,\"name\":\"%s (ESPHome)\",\"location\":\"Virtual\",\"open\":false},",
            ...
            if (tuya_mgr_get_devices(tuya_devs, TUYA_DEVICE_MAX_COUNT, &tuya_count) == ESP_OK) {
                for (int i = 0; i < tuya_count; i++) {
                    if (tuya_devs[i].virtual_zone_id > 0 && tuya_devs[i].enabled) {
                        off += snprintf(z_buf + off, 16384 - off,
                            "{\"id\":%d,\"name\":\"%s (Tuya)\",\"location\":\"Virtual\",\"open\":%s},",
        ...
        // 8. API: Zone Trigger
            ...
            if (zone_id >= 1 && zone_id <= 32) {
                for (int i = 0; i < z_count; i++) {
                    if (zones[i].id == zone_id) {
                        mock_gpio_pins[zones[i].gpio] = (mock_gpio_pins[zones[i].gpio] == 1) ? 0 : 1;
                        break;
                    }
                }
            }
            // ESPHome zones (33-64) - toggle state in device
                ...
                // In a real implementation, this would trigger ESPHome device state change
        ...
        // 11. API: Relay Toggle (POST)
            ...
            if (relay_idx >= 0 && relay_idx < r_count) {
                if (relays[relay_idx].gpio == -1) {
                    ...
                    char hostname[64];
                    uint32_t entity_key;
                    extern int esphome_api_find_entity_by_relay(int relay_id, char *hostname_out, size_t hostname_len, uint32_t *entity_key_out);
                    ...
                    if (esphome_api_find_entity_by_relay(relays[relay_idx].id, hostname, sizeof(hostname), &entity_key) == 0) {
                        ...
                        extern int esphome_api_set_switch(const char *hostname, uint32_t entity_key, bool state);
                        printf("[DEBUG] Calling esphome_api_set_switch with hostname=%s, entity_key=%" PRIu32 ", state=%s\n",
                        ...
                        if (result == 0) {
                            ...
                            int esphome_relay_idx = relays[relay_idx].id - 8;
                            if (esphome_relay_idx >= 0 && esphome_relay_idx < 24) {
                                ...
                                printf("[DEBUG] Set ESPHome relay %d (idx %d) to %s\n", 
                                    relays[relay_idx].id, esphome_relay_idx, state_b ? "ON" : "OFF");
                            }
                            found = true;
                        } else {
            ...
            if (!found) {
                ...
                if (tuya_mgr_get_devices(tuya_devs, TUYA_DEVICE_MAX_COUNT, &tuya_count) == ESP_OK) {
                    for (int i = 0; i < tuya_count && !found; i++) {
                        if (tuya_devs[i].virtual_relay_id == relay_idx && tuya_devs[i].enabled) {
        ...
        // 9. API: Network Settings (POST)
            ...
            if (mg_json_get_bool(hm->body, "$.use_dhcp", &use_dhcp)) {
                net_cfg.use_dhcp = use_dhcp;
            }
            get_json_str(hm->body, "$.ip", net_cfg.ip, sizeof(net_cfg.ip));
            get_json_str(hm->body, "$.netmask", net_cfg.netmask, sizeof(net_cfg.netmask));
            get_json_str(hm->body, "$.gateway", net_cfg.gateway, sizeof(net_cfg.gateway));
            get_json_str(hm->body, "$.dns", net_cfg.dns, sizeof(net_cfg.dns));
        ...
        // 11. API: Test Password Hashing (POST)
            ...
            if (password_hash_create(pin, &hash)) {
                char hex[256];
                password_hash_to_hex(&hash, hex);
                
                // Test verification
                bool valid = password_hash_verify(pin, &hash);
                bool invalid = password_hash_verify("9999", &hash);
                
                mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                    "{\"success\":true,\"hash\":\"%s\",\"verify_correct\":%s,\"verify_wrong\":%s}",
                    hex, valid ? "true" : "false", invalid ? "true" : "false");
            } else {
                mg_http_reply(c, 500, "Content-Type: application/json\r\n",
                    "{\"success\":false,\"error\":\"Hash creation failed\"}");
            }
        }
        // 12. API: Test Session Management (POST)
            ...
            if (session_create(username, "127.0.0.1", token)) {
                ...
                mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                    "{\"success\":true,\"token\":\"%s\",\"validated\":%s,\"user\":\"%s\"}",
        ...
        // 17. API: ESPHome Trigger Sensor (POST)
            ...
            extern void mock_esphome_trigger_sensor(const char *entity_id, bool state);
        ...
        // 18. API: ESPHome Control Switch (POST)
            ...
            for (int i = 0; i < esphome_count; i++) {
                ...
                if (strcmp(hostname_short, entity_id) == 0 || 
                    ...
                    if (dev->enabled && dev->entity_key != 0) {
                        ...
                        real_result = esphome_api_set_switch(dev->hostname, dev->entity_key, state);
                        if (real_result == 0) {
            ...
            mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                "{\"status\":\"success\",\"entity\":\"%s\",\"state\":%s,\"method\":\"%s\"}", 
                entity_id, state ? "true" : "false", 
            ...
            for (int i = 0; i < cam_count; i++) {
                ...
                if (cam) {
                    offset += snprintf(response + offset, sizeof(response) - offset,
                        "%s{\"id\":%d,\"hostname\":\"%s\",\"port\":%d,\"friendly_name\":\"%s\",\"esphome_id\":\"%s\",\"enabled\":%s,\"sd_recording_enabled\":%s,\"last_snapshot\":%ld,\"failures\":%d}",
                        i > 0 ? "," : "",
                        i, cam->hostname, cam->port, cam->friendly_name, cam->camera_id,
                        cam->enabled ? "true" : "false",
                        cam->sd_recording_enabled ? "true" : "false",
                        (long)cam->last_snapshot_time,
                        cam->consecutive_failures);
                }
            }
            snprintf(response + offset, sizeof(response) - offset, 
                "],\"sd_available\":%s,\"stats\":{\"total_snapshots\":0,\"alarm_triggered_snapshots\":0}}", 
        ...
        // GET /api/camera/:id/snapshot - Get camera snapshot
            ...
            if (id_start) {
                ...
                if (camera_mgr_fetch_snapshot(camera_id, &buffer, &size) == 0 && buffer && size > 0) {
        ...
        // GET /api/camera/:id/stream - MJPEG streaming proxy (mock)
            ...
            if (id_start) {
                ...
                printf("[MOCK] MJPEG stream proxy for camera %d (%s)\n", camera_id, cam->friendly_name);
                if (strlen(cam->stream_url) == 0) {
                    mg_http_reply(c, 500, "Content-Type: application/json\r\n", "{\"error\":\"No stream_url configured\"}");
                    return;
                }
                char cmd[512];
                snprintf(cmd, sizeof(cmd), "curl -s --max-time 30 --connect-timeout 5 '%s'", cam->stream_url);
                FILE *fp = popen(cmd, "r");
                if (!fp) {
        ...
        // POST /api/camera/add - Add new camera
            ...
            if (strlen(hostname) > 0 && strlen(name) > 0) {
                int camera_id_result = camera_mgr_add_device(hostname, port, name, 
                    ...
                    strlen(motion_cmd) > 0 ? motion_cmd : "",
                    enabled);
                if (camera_id_result >= 0) {
                    mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                        "{\"success\":true,\"camera_id\":%d}", camera_id_result);
                } else {
                    mg_http_reply(c, 500, "Content-Type: application/json\r\n", 
        ...
        // GET /api/tuya/devices - List all Tuya devices
            ...
            for (int i = 0; i < count; i++) {
                if (i > 0) mg_printf(c, ",");
                mg_printf(c, "{\"id\":\"%s\",\"name\":\"%s\",\"type\":\"%s\","
                            "\"state\":\"%s\",\"enabled\":%s",
                         devices[i].device_id, devices[i].name,
                         tuya_device_type_name(devices[i].type),
                         devices[i].state == TUYA_DEV_STATE_ONLINE ? "online" : "offline",
                ...
                if (devices[i].capabilities & TUYA_CAP_SWITCH) {
                    mg_printf(c, ",\"switch_on\":%s", devices[i].switch_on ? "true" : "false");
                }
                ...
                if (devices[i].capabilities & TUYA_CAP_CONTACT) {
                    mg_printf(c, ",\"contact_open\":%s", devices[i].contact_open ? "true" : "false");
                }
                if (devices[i].capabilities & TUYA_CAP_MOTION) {
                    mg_printf(c, ",\"motion\":%s", devices[i].motion_detected ? "true" : "false");
                }
        ...
        // DELETE /api/tuya/device/:id
            ...
            if (id_start) {
                ...
                int id_len = 0;
                while (id_start[id_len] && id_start[id_len] != '/' && id_len < TUYA_DEVICE_ID_MAX_LEN-1) {
        ...
        // POST /api/tuya/device/:id/control
            ...
            if (id_start) {
                ...
                if (act) { strncpy(action, act, sizeof(action)-1); free(act); }
                if (val) { strncpy(value, val, sizeof(value)-1); free(val); }
...
//    storage_debug_print(); 
    ...
    printf("[SECURITY] Checking for users needing password migration...\n");
    int migration_result = users_migrate_all_to_hash();
    if (migration_result == 0) {  // ESP_OK is 0
        printf("[SECURITY] User migration complete - saving to storage\n");
        storage_save_users();
    }
    ...
    printf(">> Matter Trigger: curl -X POST http://localhost:8000/api/matter/trigger -d '{\"zone_id\":33}'\n");
    printf("\n>> ESPHome Test: curl http://localhost:8000/api/esphome/devices\n");
    printf(">> ESPHome Trigger: curl -X POST http://localhost:8000/api/esphome/trigger -d '{\"entity_id\":\"basement_motion\",\"state\":true}'\n");
    printf(">> ESPHome Control: curl -X POST http://localhost:8000/api/esphome/control -d '{\"entity_id\":\"kitchen_light\",\"state\":true}'\n");
    printf("\n>> Camera Test: curl http://localhost:8000/api/cameras\n");
    printf(">> Camera Add: curl -X POST http://localhost:8000/api/camera/add -d '{\"hostname\":\"192.168.1.100\",\"port\":80,\"friendly_name\":\"Test Cam\",\"esphome_id\":\"camera\",\"enabled\":true}'\n");
    printf(">> Camera Snapshot: curl http://localhost:8000/api/camera/0/snapshot --output snapshot.jpg\n");

    // HTTP listener for compatibility
    mg_http_listen(&mgr, "http://0.0.0.0:8000", web_handler, NULL);
    
    // HTTPS listener on port 8443 (higher port for testing, pass non-NULL fn_data for TLS init)
    mg_http_listen(&mgr, "https://0.0.0.0:8443", web_handler, (void *)1);
    
    // ESPHome Native API listener on port 6053 (reject ESPHome connections gracefully)
    ...
    for(int i=0; i<100; i++) last_pin_state[i] = mock_gpio_pins[i];
    ...
    while (1) {
        ...
        if (read(0, &ch, 1) > 0) {
            if (ch == 10 || ch == 13) {
                ...
                if (pin_idx > 0) engine_handle_keypad_input(pin_buffer); 
                pin_idx = 0; printf("\n");
            } 
            else if (ch == 127 || ch == 8) {
                if (pin_idx > 0) { pin_idx--; printf("\b \b"); fflush(stdout); }
            }
        ...
        // Noonlight Sync
        if (config.notify == 4 && ++nl_sync_counter >= 400) {
            nl_sync_counter = 0;
            noonlight_sync_task(&config);
        }   
