...
esp_err_t zigbee_mgr_init(const void *config, void *callback) {
    ...
    // evice 3: Smart Outlet (0x4567)
    // Device 3: Smart Outlet (0x4567) removed relay 65 mapping
    dev = &s_mock.devices[s_mock.device_count++];
    ...
    dev->endpoint = 1;
    dev->relay_id = -1;  // No relay mapping
...
esp_err_t zigbee_mgr_update_device(uint16_t short_addr, const char *name,
    ...
    printf("ZIGBEE_MOCK: Update device 0x%04X\\n", short_addr);
    
    for (int i = 0; i < s_mock.device_count; i++) {
        if (s_mock.devices[i].short_addr == short_addr) {
            // Update fields if provided
            if (name != NULL) {
                strncpy(s_mock.devices[i].name, name, ZIGBEE_MAX_NAME_LEN - 1);
                s_mock.devices[i].name[ZIGBEE_MAX_NAME_LEN - 1] = '\0';
            }
            if (zone_id >= 0) {
            ...
            // Note: enabled field not in zigbee_device_t struct, using state as proxy
            if (enabled >= 0) {
                s_mock.devices[i].state = enabled ? ZIGBEE_STATE_ONLINE : ZIGBEE_STATE_OFFLINE;
            }
            
            printf("ZIGBEE_MOCK: Updated device 0x%04X: name=%s, zone=%d, relay=%d\\n",
                   short_addr, s_mock.devices[i].name, s_mock.devices[i].zone_id,
                   s_mock.devices[i].relay_id);
            return ESP_OK;
        }
    }
