...
// Project Headers
#include "audit_log.h" // Audit log for security events
#include "cJSON.h"
  ...
  if (ev == MG_EV_ACCEPT && fn_data != NULL) {
    ...
    if (mg_strcmp(hm->uri, mg_str("/api/users")) == 0) {
      ...
      if (strlen(name) > 0 && strlen(totp_secret) > 0) {
        // Find user email
        char email_copy[STR_MEDIUM] = {0};
        storage_lock();
        for (int i = 0; i < storage_get_user_count(); i++) {
          if (strcmp(storage_get_user(i)->name, name) == 0) {
            const char *email_ptr = storage_get_user(i)->email;
            if (email_ptr) {
              strncpy(email_copy, email_ptr, sizeof(email_copy) - 1);
            }
            break;
          }
        }
        char account_id[STR_MEDIUM] = {0};
        strncpy(account_id, storage_get_config()->account_id,
                sizeof(account_id) - 1);
        storage_unlock();
    ...
    // 3. API: Full Status (Matches index.html expectations)
      ...
      if (cfg) {
        ...
        cJSON_AddStringToObject(cfg, "acct_id",
                                storage_get_config()->account_id);
        ...
        cJSON_AddStringToObject(cfg, "violation_type",
      ...
      if (z_arr) {
        ...
        for (int i = 0; i < storage_get_zone_count(); i++) {
          if (storage_get_zone(i)->gpio >= 0) { // Only physical zones with GPIO
            ...
            cJSON_AddStringToObject(z, "description",
                                    storage_get_zone(i)->description);
        ...
        for (int i = 0; i < storage_get_esphome_count(); i++) {
          if (storage_get_esphome_device(i)->virtual_zone_start > 0 &&
            ...
            cJSON_AddNumberToObject(
                z, "id", storage_get_esphome_device(i)->virtual_zone_start);
            cJSON_AddStringToObject(
                z, "name", storage_get_esphome_device(i)->friendly_name);
            cJSON_AddStringToObject(
                z, "description",
      ...
      if (r_arr) {
        ...
        for (int i = 0; i < storage_get_relay_count(); i++) {
          if (storage_get_relay(i)->id <= 8) { // Only physical relays
            ...
            cJSON_AddStringToObject(r, "location",
                                    storage_get_relay(i)->location);
            cJSON_AddBoolToObject(
                r, "active", hal_get_relay_state(storage_get_relay(i)->gpio));
        ...
        for (int i = 0; i < storage_get_esphome_count(); i++) {
          if (storage_get_esphome_device(i)->virtual_relay_start > 0 &&
            ...
            if (!r)
              continue;
            cJSON_AddNumberToObject(
                r, "id", storage_get_esphome_device(i)->virtual_relay_start);
            cJSON_AddStringToObject(
                r, "name", storage_get_esphome_device(i)->friendly_name);
            cJSON_AddStringToObject(r, "location",
                                    storage_get_esphome_device(i)->location);
    ...
    // API: Firmware Version
      ...
      cJSON_AddStringToObject(root, "boot_partition",
      ...
      if (lines) {
        ...
        for (int i = 0; i < MAX_LOG_LINES; i++) {
          ...
          if (g_log_seqs[idx] > seq) {
            cJSON *str_val = cJSON_CreateString(g_log_buf[idx]);
            if (str_val)
              cJSON_AddItemToArray(lines, str_val);
          }
        }
      }
      char *json_str = cJSON_PrintUnformatted(root);
    ...
    // API: Get Audit Log (for audit.html)
    else if (mg_strcmp(hm->uri, mg_str("/api/audit")) == 0) {
      // This endpoint is present in the Linux mock but was missing here.
      // It serves the audit log data to be displayed on audit.html.
      char *json_buf = malloc(8192); // Allocate buffer for safety on ESP32
      if (json_buf == NULL) {
        mg_http_reply(c, 500, "Content-Type: application/json\r\n",
                      "{\"error\":\"out of memory\"}");
        return;
      }
      // The implementation of audit_log_to_json is in the missing audit_log.c
      if (audit_log_to_json(json_buf, 8192, 50)) {
        mg_http_reply(c, 200, "Content-Type: application/json\r\n", "%s",
                      json_buf);
      } else {
        mg_http_reply(c, 500, "Content-Type: application/json\r\n",
    ...
    if (mg_strcmp(hm->uri, mg_str("/api/zones")) == 0) {
      ...
      for (int i = 0; i < storage_get_zone_count(); i++) {
        ...
        cJSON_AddStringToObject(z, "location", "");
        cJSON_AddBoolToObject(
            z, "open", hal_get_zone_state(storage_get_zone(i)->gpio) == 0);
        cJSON_AddItemToArray(z_arr, z);
      }
    ...
    if (mg_strcmp(hm->uri, mg_str("/api/login")) == 0) {
      ...
      if (user_authenticate_pin(pin_in, &user) && user != NULL) {
        ...
        if (session_create(user->name, client_ip, token)) {
          ESP_LOGI(TAG, "Session created for user '%s' from IP %s", user->name,
                   client_ip);
          mg_http_reply(c, 200, "Content-Type: application/json\r\n",
    ...
    // API: OTA Firmware Upload (Secure)
      ...
      if (auth_hdr && auth_hdr->len > 7) {
        ...
        if (strncmp(auth_hdr->buf, "Bearer ", 7) == 0) {
          ...
          if (pin_len < sizeof(pin_in)) {
            snprintf(pin_in, sizeof(pin_in), "%.*s", (int)pin_len,
                     auth_hdr->buf + 7);
          }
        }
      } else {
        ...
        if (strcmp(pin_in, storage_get_config()->pin) == 0) {
          ...
          for (int i = 0; i < storage_get_user_count(); i++) {
            if (strcmp(storage_get_user(i)->pin, pin_in) == 0) {
              if (storage_get_user(i)->is_admin) {
                authenticated = true;
                break;
              }
            }
      ...
      long offset = mg_http_upload(c, hm, &mg_fs_posix, "/tmp/firmware.bin",
                                   10 * 1024 * 1024);
      ...
      if (offset < 0) {
        ...
        if (img_hdr.magic != ESP_IMAGE_HEADER_MAGIC) {
          ...
          fclose(fw_file);
          mg_http_reply(c, 400, "Content-Type: application/json\r\n",
                        "{\"status\":\"error\",\"message\":\"Invalid firmware "
                        "magic number\"}");
          return;
        }
        ...
        if (!buffer) {
          ...
          mg_http_reply(c, 500, "Content-Type: application/json\r\n",
                        "{\"status\":\"error\",\"message\":\"Memory allocation "
                        "failed\"}");
          return;
        }
        ...
        if (err != ESP_OK) {
          ...
          ota_in_progress = false;
          mg_http_reply(c, 500, "Content-Type: application/json\r\n",
                        "{\"status\":\"error\",\"message\":\"Failed to set "
                        "boot partition\"}");
          return;
        }
        ...
        mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                      "{\"status\":\"success\",\"message\":\"Firmware updated, "
                      "restarting in 3 seconds\",\"bytes\":%zu}",
                      ota_bytes_written);
    ...
    // API: Relay Toggle
      ...
      for (int i = 0; i < storage_get_relay_count() && !found; i++) {
        if (current_idx == relay_idx) {
          ...
          ESP_LOGI(TAG, "Physical relay %d (%s) set to %s",
    ...
    // API: RF Inject (Simulate RF code)
      ...
      // Note: In a real system, you might want to trigger the engine's RF
      // handler here
      mg_http_reply(c, 200, "Content-Type: application/json\r\n",
                    "{\"status\":\"ok\"}");
    }
    ...
    // GET /api/cameras - List all cameras
      ...
      if (cameras_arr) {
        ...
        for (int i = 0; i < cam_count; i++) {
          ...
          if (cam) {
            ...
            cJSON_AddStringToObject(cam_obj, "friendly_name",
                                    cam->friendly_name);
            cJSON_AddStringToObject(cam_obj, "esphome_id", cam->esphome_id);
            cJSON_AddBoolToObject(cam_obj, "enabled", cam->enabled);
            cJSON_AddBoolToObject(cam_obj, "sd_recording_enabled",
                                  cam->sd_recording_enabled);
            cJSON_AddNumberToObject(cam_obj, "last_snapshot",
                                    (double)cam->last_snapshot_time);
            cJSON_AddNumberToObject(cam_obj, "failures",
      ...
      if (stats_obj) {
        ...
        cJSON_AddNumberToObject(stats_obj, "total_snapshots",
                                cam_stats.total_snapshots);
        cJSON_AddNumberToObject(stats_obj, "alarm_triggered_snapshots",
                                cam_stats.alarm_triggered_snapshots);
      }
      cJSON_AddBoolToObject(response, "sd_available",
                            camera_mgr_is_sd_available());
    ...
    // GET /api/camera/:id/stream - MJPEG streaming proxy
      ...
      if (id_start) {
        ...
        ESP_LOGI(TAG, "Proxying MJPEG stream from camera %d: %s", camera_id,
                 mjpeg_url);
        ...
        ESP_LOGW(
            TAG,
            "MJPEG streaming proxy not fully implemented yet, use snapshots");
        mg_printf(c, "--frame\r\n"
                     "Content-Type: text/plain\r\n"
                     "\r\n"
                     "MJPEG streaming coming soon. Please use snapshot mode "
                     "for now.\r\n");
        c->is_draining = 1;
      } else {
        mg_http_reply(c, 400, "Content-Type: application/json\r\n",
    ...
    // POST /api/camera/add - Add new camera
      ...
      if (!hostname || !cJSON_IsString(hostname) || !name ||
          !cJSON_IsString(name)) {
        cJSON_Delete(root);
        mg_http_reply(c, 400, "Content-Type: application/json\r\n",
      ...
      int camera_id = camera_mgr_add_device(
          hostname->valuestring, port ? port->valueint : 80, name->valuestring,
          esphome_id && cJSON_IsString(esphome_id) ? esphome_id->valuestring
                                                   : "camera",
          username && cJSON_IsString(username) ? username->valuestring : "",
          password && cJSON_IsString(password) ? password->valuestring : "",
          snapshot_cmd && cJSON_IsString(snapshot_cmd)
    ...
    // POST /api/camera/:id/sd_recording - Toggle SD recording
      ...
      if (id_start) {
        ...
        const camera_device_t *cam = camera_mgr_get_device(camera_id);
        if (cam) {
          esp_err_t err = camera_mgr_set_sd_recording(
              camera_id, !cam->sd_recording_enabled);
          if (err == ESP_OK) {
            mg_http_reply(c, 200, "Content-Type: application/json\r\n",
      ...
      if (ret != ESP_OK) {
        ...
        if (devices_array) {
          ...
          for (int i = 0; i < count; i++) {
            ...
            cJSON_AddStringToObject(dev, "type",
            ...
            cJSON_AddStringToObject(
                ...
                : devices[i].state == TUYA_DEV_STATE_PAIRING ? "pairing"
                                                             : "error");
            cJSON_AddBoolToObject(dev, "enabled", devices[i].enabled);

            // Mappings
            if (devices[i].virtual_zone_id >= 0) {
              cJSON_AddNumberToObject(dev, "zone_id",
                                      devices[i].virtual_zone_id);
            }
            if (devices[i].virtual_relay_id >= 0) {
              cJSON_AddNumberToObject(dev, "relay_id",
                                      devices[i].virtual_relay_id);
            }
            ...
            if (devices[i].capabilities & TUYA_CAP_CONTACT) {
              cJSON_AddBoolToObject(dev, "contact_open",
                                    devices[i].contact_open);
            }
    ...
    // DELETE /api/tuya/device/:id - Remove device
      ...
      if (mg_match(hm->uri, mg_str("/api/tuya/device/*"), caps)) {
        ...
        snprintf(device_id, sizeof(device_id), "%.*s", (int)caps[0].len,
                 caps[0].buf);
...
// ============================================================
      ...
      if (scheduler_get_all_tasks(tasks, &count) == ESP_OK) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          if (tasks[i].type == SCHEDULE_INTERVAL) {
            cJSON_AddNumberToObject(task_obj, "interval",
                                    tasks[i].schedule.interval_seconds);
          } else if (tasks[i].type == SCHEDULE_ONCE) {
            cJSON_AddNumberToObject(task_obj, "run_time",
                                    tasks[i].schedule.once_time);
          }
      ...
      if (body.len > 0) {
        ...
        if (json_str) {
          ...
          if (json) {
            ...
            uint8_t task_id;
            esp_err_t result = ESP_FAIL;
            int prio = priority && cJSON_IsNumber(priority)
                           ? priority->valueint
                           : SCHEDULE_PRIORITY_NORMAL;

            if (name && type && cJSON_IsNumber(type)) {
              if (type->valueint == SCHEDULE_INTERVAL && interval &&
                  cJSON_IsNumber(interval)) {
                result = scheduler_add_interval_task(name->valuestring,
                                                     interval->valueint, NULL,
                                                     NULL, prio, &task_id);
              } else if (type->valueint == SCHEDULE_CRON && cron_expr &&
                         cJSON_IsString(cron_expr)) {
                result = scheduler_add_cron_task(name->valuestring,
                                                 cron_expr->valuestring, NULL,
                                                 NULL, prio, &task_id);
              } else if (type->valueint == SCHEDULE_ONCE && run_time &&
                         cJSON_IsNumber(run_time)) {
                result = scheduler_add_once_task(name->valuestring,
                                                 run_time->valueint, NULL, NULL,
                                                 prio, &task_id);
              }
                ...
                if (action_type && cJSON_IsNumber(action_type)) {
                  ...
                  case SCHEDULER_ACTION_GROUP_CONTROL: {
                    ...
                    if (group_id && cJSON_IsNumber(group_id)) {
                      ...
                      task_action.params.group.on =
                          (on && cJSON_IsBool(on)) ? cJSON_IsTrue(on) : true;
                      task_action.params.group.level =
                          (level && cJSON_IsNumber(level)) ? level->valueint
                                                           : 0;
                    }
                    break;
                  }
                  ...
                  case SCHEDULER_ACTION_MQTT_PUBLISH: {
                    ...
                    if (payload && cJSON_IsString(payload)) {
                      strncpy(task_action.params.mqtt.payload,
                              payload->valuestring,
                  ...
                  case SCHEDULER_ACTION_HTTP_REQUEST: {
                    ...
                    if (method && cJSON_IsString(method)) {
                      strncpy(task_action.params.http.method,
                              method->valuestring,
    ...
    else if (mg_match(hm->uri, mg_str("/api/scheduler/tasks/*"), NULL) &&
          ...
          if (json) {
            ...
            if (enabled && cJSON_IsBool(enabled)) {
              esp_err_t result = cJSON_IsTrue(enabled)
                                     ...
                                     : scheduler_disable_task(task_id);
    ...
    else if (mg_match(hm->uri, mg_str("/api/scheduler/tasks/*/trigger"),
                      NULL)) {
      const char *uri_buf = hm->uri.buf;
      const char *id_start = uri_buf + strlen("/api/scheduler/tasks/");
      char id_str[16];
...
void app_main(void) {
  ...
  if (!session_mgr_init()) {
    ESP_LOGE(
        TAG,
        "Failed to initialize session manager - continuing without tokens");
  } else {
  ...
  if (!audit_log_init()) {
    ESP_LOGE(TAG, "Failed to initialize audit log");
  } else {
  ...
  if (eth_ret != ESP_OK) {
    ESP_LOGE(TAG, "Ethernet initialization failed: %s",
  ...
  */
  ...
  if (camera_mgr_init() != ESP_OK) {
    ...
    if (camera_mgr_load_config() != ESP_OK) {
      ...
      ESP_LOGI(TAG, "Camera manager initialized with %d camera(s)",
  ...
  */
  ...
  ESP_LOGI(TAG, "To enable: Install ESP-Matter SDK and set
  CONFIG_ENABLE_MATTER_CONTROLLER=y"); #endif
  */
